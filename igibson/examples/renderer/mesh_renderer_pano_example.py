...
def main(selection="user", headless=False, short_exec=False):
    ...
    if len(sys.argv) > 1:
        ...
        model_path = os.path.join(get_scene_path("Hanson"), "mesh_z_up.obj")

    # Create renderer object and load the scene model
    settings = MeshRendererSettings(enable_pbr=False,optimized=True)
    ...
    max_steps = -1 if not short_exec else 2000
    step = 0

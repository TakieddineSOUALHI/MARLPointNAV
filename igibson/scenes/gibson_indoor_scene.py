...
class StaticIndoorScene(IndoorScene):
    ...
    def load_scene_mesh(self, simulator):
        ...
        Load scene mesh
        """
        textured_mesh = os.path.join(get_scene_path(self.scene_id), "{}_mesh_texture.obj".format(self.scene_id))
        ...
        if not os.path.isfile(filename):
            filename = textured_mesh

        collision_id = p.createCollisionShape(p.GEOM_MESH, fileName=filename, flags=p.GEOM_FORCE_CONCAVE_TRIMESH)

        # Use the textured mesh for the iGibson renderer so UV coordinates are available.
        # Fall back to the collision mesh if the textured mesh does not exist.
        visual_mesh = textured_mesh if os.path.isfile(textured_mesh) else filename

        # Always create a visual shape so load_object_in_renderer can retrieve the mesh
        # path via p.getVisualShapeData — even in headless mode the visual shape is only
        # a PyBullet data structure and has no rendering overhead.
        visual_id = p.createVisualShape(p.GEOM_MESH, fileName=visual_mesh)
        ...
        if simulator.use_pb_gui and self.pybullet_load_texture:
            texture_filename = get_texture_file(visual_mesh)

...
class BaseTask:
    ...
    def get_reward(self, env, collision_links=[], action=None, info={}):
        ...
        """
        rewards= [0 for i in range(env.robots_num)]
        for reward_function in self.reward_functions:
            reward = reward_function.get_reward(self, env)
            rewards=[rewards[i] + reward[i] for i in range(env.robots_num)]
    ...
    def get_termination(self, env, collision_links=[], action=None, info={}):
        ...
        d,s = self.termination_conditions[1].get_termination(self, env)
        done = d
        success = s
        
        for i in range(env.robots_num):
            info["done"+str(i)] = done[i]
            info["success"+str(i)] = success[i]
        return done, info
